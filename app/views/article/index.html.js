...
'scbd-angularjs-services/generic-service', 'js/pin-grid'], function (app) {
    ...
    return ['$scope', '$http', 'IGenericService', '$q', '$location', '$timeout',
        function ($scope, $http, genericService, $q, $location, $timeout) {
        
            $q.when(genericService.query('v2017', 'articles'))
                .then(function (data) {
                    $scope.articles = (function () {
                        var articles = [];
                        for (var i = 0; i < data.length; i++) {
                            articles.push(_.extend(data[i], {
                                color: randomColor(),
                                colspan: randomSpan(),
                                rowspan: randomSpan()
                            }));
