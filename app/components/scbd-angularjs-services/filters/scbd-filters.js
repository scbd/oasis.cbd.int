...
import _ from 'lodash';
  ...
  app.filter("term", ["$http", '$filter', function ($http, $filter) {
    ...
    return function (term, locale) {
      ...
      else if(term && term._id){
        term.identifier = term._id;
      }
      locale = locale || "en";
  ...
  //
  //============================================================
  app.filter("tagTerm", ["$http", '$filter', function ($http, $filter) {
    var cacheMap = {};

    return function (searchTerm, table, field, locale) {

      var term = angular.copy(searchTerm)
      if (!term)
        return "";
      ...
      if(!table || table=='')
          throw Error("table not specified for tagTerm filter");
      ...
      cacheMap[term.identifier] = $http.get('/api/v2017/'+table+'/' + encodeURIComponent(term.identifier)+'?f={"'+field+'":1}', {cache: true})
      .then(function (result) {
